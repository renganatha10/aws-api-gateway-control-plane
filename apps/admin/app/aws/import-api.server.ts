import { CreateDeploymentCommand, ImportRestApiCommand, PutRestApiCommand } from "@aws-sdk/client-api-gateway"
...
export async function importApiSpec(specJson: Record<string, unknown>): Promise<string> {
  ...
  const result = await apigwClient.send(new ImportRestApiCommand({
    body,
...
export async function putApiSpec(restApiId: string, specJson: Record<string, unknown>): Promise<void> {
  const body = Buffer.from(JSON.stringify(specJson))
  await apigwClient.send(new PutRestApiCommand({
    restApiId,
    body,
    mode: "overwrite",
    failOnWarnings: false,

...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (!invokeUrl || !url.startsWith(invokeUrl)) {
    return Response.json({ error: "Target URL is not authorized for this consumer" }, { status: 403 })
  }
  ...
  try {
    ...
    return Response.json({ error: "Request failed — could not reach the endpoint.", ms: Date.now() - start })
  }

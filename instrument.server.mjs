...
// instrument-hook.mjs (loaded first via --import) already registered the OTel
// ESM hook before this file was parsed, so pino and http are properly patched.
...
// Create the shared pino logger. PinoInstrumentation patches pino at import
// time (via the ESM hook in instrument-hook.mjs), so this logger instance
// automatically injects trace_id + span_id into every log record.
const logger = pino({
...
// Console shim -- active from process start, covers all call sites.

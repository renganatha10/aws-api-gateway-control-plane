...
// Credentials provisioned against LocalStack Cognito in global-setup.ts
const TEST_EMAIL = process.env.TEST_USER_EMAIL ?? "test@e2e.local";
...
test.describe("Login page", () => {
  ...
  test("signs in with valid credentials via LocalStack Cognito", async ({ page }) => {
    await page.goto("/login");
    await page.getByLabel("Email").fill(TEST_EMAIL);
    await page.getByLabel("Password").fill(TEST_PASSWORD);
    await page.getByRole("button", { name: "Sign In" }).click();
    // Successful auth redirects away from /login

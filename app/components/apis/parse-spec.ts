...
export function parseSpec(
  yamlStr: string,
): { spec: RawSpec; groups: EndpointGroup[]; hosts: Record<string, string> } {
  let spec: RawSpec = {}
  try {
  ...
  for (const [path, methods] of Object.entries(spec.paths ?? {})) {
    for (const [method, op] of Object.entries(methods)) {
      ...
      if (bodyParam && rawBodyParam) {
        ...
        if (rawSchema) {
          try { bodySample = JSON.stringify(generateSample(spec, rawSchema), null, 2) }
        ...
        if (ctObj?.schema) {
          try { bodySample = JSON.stringify(generateSample(spec, ctObj.schema), null, 2) }
          catch { bodySample = null }
        }
      }

...
import { apiAssociations, apis, type NewApiAssociation, products } from "~/lib/schema";
...
export async function listApiScopesForProduct(productId: number) {
  return db
    ...
    .where(
      and(eq(apiAssociations.productId, productId), isNotNull(apis.scope), isNotNull(apis.awsApiId))
    );
}

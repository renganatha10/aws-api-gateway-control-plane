...
import { type Domain, domainRouteMappings, domains, type NewDomain } from "~/lib/schema";
...
export async function updateDomainAwsTarget(id: number, awsDomainName: string): Promise<void> {
  await db.update(domains).set({ awsDomainName, updatedAt: new Date() }).where(eq(domains.id, id));
}

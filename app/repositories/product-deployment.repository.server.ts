...
import { type NewProductDeployment, productDeployments } from "~/lib/schema";
...
export async function listDeploymentsByProduct(productId: number) {
  return db.select().from(productDeployments).where(eq(productDeployments.productId, productId));
}

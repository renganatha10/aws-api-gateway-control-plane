...
import { environments, organisations, type Organisation, type NewOrganisation } from "~/lib/schema"
  ...
  return db.transaction(async (tx) => {
    ...
    if (envNames.length > 0) {
      await tx.insert(environments).values(
        envNames.map((name) => ({ name, organisationId: created.id, createdBy: organisation.createdBy })),
      )
    }
    return created
  })
...
export async function listOrganisations(createdBy: string): Promise<Organisation[]> {
  return db.select().from(organisations).where(eq(organisations.createdBy, createdBy)).orderBy(organisations.createdAt)
}
...
export async function countOrganisations(createdBy: string): Promise<number> {
  const [{ value }] = await db.select({ value: count() }).from(organisations).where(eq(organisations.createdBy, createdBy))
  return Number(value)
}
